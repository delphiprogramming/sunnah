...
if (Meteor.isServer) {
	Meteor.startup(function () {
		if (Langs.find({}).count() === 0) {
			...
			Langs.insert({order: 1, ref: 'en', name: 'English', native: 'English', active: true});
			Langs.insert({order: 2, ref: 'id', name: 'Indonesian', native: 'Bahasa Indonesia', active: true});
			Langs.insert({order: 3, ref: 'ar', name: 'Arabic', native: 'العَرَبِية‎', active: true});
		}
	});
}
