...
limitAyatRules = function(lang, sno) {
	var query = {};
	query['text.'+lang] = {$exists:true};
	query['surah'] = sno;
	return query;
...
ayatQuran = function(sno) {
	var query = {};
	query['text.ar'] = {$exists:true};
	query['surah'] = sno;
	return query;
}
...
if (Meteor.isServer) {
	...
	Meteor.publish('editAyat', function (lang, sno) {
		return Ayat.find(limitAyatRules(lang, sno));
	});
	Meteor.publish('ayatQuran', function (sno) {
		return Ayat.find(ayatQuran(sno));
	});
}
...
if (Meteor.isServer) {
	Meteor.startup(function () {
		...
		if (Ayat.find({}).count() === 0) {
			var quranAr = require('../imports/startup/quran/quran-ar.json');
			var quranEn = require('../imports/startup/quran/quran-en.json');
			var quranId = require('../imports/startup/quran/quran-id.json');
